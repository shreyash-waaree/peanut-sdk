...
/* JADX INFO: loaded from: peanut-sdk-release.aar:classes.jar:com/keenon/sdk/sensor/ota/impl/RosOTAImplV2.class */
public class RosOTAImplV2 implements IOta {
    ...
    private IDataCallback mVersionCallback = new IDataCallback() { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.5
        @Override // com.keenon.sdk.external.IDataCallback
        public void success(String result) {
            RosOTAImplV2.this.log("queryVersion success", result);
            ...
            if (curVersion == null || curVersion.length() == 0) {
                RosOTAImplV2.this.notifyError(PeanutError.OTA_UNKNOWN_VERSION);
                RosOTAImplV2.this.updateCheckStatus(RosOTAImplV2.STATUS_CODE_QUERY_VERSION_ERROR, true);
            } else {
                RosOTAImplV2.this.mConfig = RosOTAImplV2.this.mConfig.newBuilder().withCurVersion(curVersion).build();
                RosOTAImplV2.this.updateCheckStatus(10001, false);
            }
        }

        @Override // com.keenon.sdk.external.IDataCallback
        public void error(ApiError error) {
            RosOTAImplV2.this.loge("queryVersion error", error.toString());
            RosOTAImplV2.this.notifyError(PeanutError.COAP_RESPONSE_ERROR);
            RosOTAImplV2.this.updateCheckStatus(RosOTAImplV2.STATUS_CODE_QUERY_VERSION_ERROR, true);
        }
    };
    private IDataCallback mStatusCallback = new IDataCallback() { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.6
        @Override // com.keenon.sdk.external.IDataCallback
        public void success(String result) {
            if (!RosOTAImplV2.this.isReleased()) {
                RosOTAImplV2.this.log("subscribe upgrade status success", result);
                UpgradeStatusApi.Bean bean = (UpgradeStatusApi.Bean) GsonUtil.gson2Bean(result, UpgradeStatusApi.Bean.class);
                if (bean == null || bean.getData() == null) {
                    return;
                }
                if (bean.getCode() != 0) {
                    RosOTAImplV2.this.notifyError(bean.getCode());
                    RosOTAImplV2.this.updateCheckStatus(RosOTAImplV2.STATUS_CODE_SUB_STATUS_ERROR, true);
                } else {
                    RosOTAImplV2.this.updateCheckStatus(10002, false);
                }
                RosOTAImplV2.this.notifyProgressChanged(bean.getData().getPercent());
                RosOTAImplV2.this.handleDeviceUpgradeStatus(bean.getData().getStatus());
            }
        }

        @Override // com.keenon.sdk.external.IDataCallback
        public void error(ApiError error) {
            RosOTAImplV2.this.loge("subscribe upgrade status error", error.toString());
            RosOTAImplV2.this.notifyError(error.getCode());
            RosOTAImplV2.this.updateCheckStatus(RosOTAImplV2.STATUS_CODE_SUB_STATUS_ERROR, true);
        }
    };
    ...
    private void updatePackage() {
        ...
        this.mPackage.setUrl(this.mConfig.fileDirectory() + "/" + this.mConfig.fileName());
    ...
    public boolean checkFile() {
        ...
        File otaFile = this.mConfig.file();
        if (null == otaFile && StringUtils.isNotEmpty(this.mConfig.fileDirectory()) && StringUtils.isNotEmpty(this.mConfig.fileName())) {
            otaFile = new File(this.mConfig.fileDirectory() + File.separator + this.mConfig.fileName());
        }
        if (null == otaFile) {
            loge("checkFile", "文件不存在");
            notifyError(PeanutError.OTA_FW_NOT_EXIST);
            return false;
        }
        long sourceLen = otaFile.length();
        File otaFile2 = copyFile(otaFile);
        if (!otaFile2.exists() || otaFile2.length() != sourceLen) {
            loge("checkFile", otaFile2.getAbsolutePath() + "文件不存在");
            notifyError(PeanutError.OTA_FW_NOT_EXIST);
            return false;
        }
        builder.withFile(otaFile2).withFileName(otaFile2.getName()).withFileDirectory(otaFile2.getParent()).withFileSize(otaFile2.length());
    ...
    private File copyFile(File sourceFile) {
        log("copy file : " + sourceFile.getAbsolutePath());
        File destFile = new File("/data/upgrade/ros/" + sourceFile.getName());
        try {
    ...
    private void queryVersion() {
        this.mDispatcher.enqueue(new Dispatcher.NodeCall("检查设备版本信息", "", true, this.mDispatcher) { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.1
            @Override // com.keenon.common.utils.Dispatcher.NodeCall
            protected void execute() {
                new DeviceVersionApi().send(RosOTAImplV2.this.mVersionCallback);
            }

            @Override // com.keenon.common.utils.Dispatcher.NodeCall
            protected boolean interceptExecute() {
                return RosOTAImplV2.this.mPreCheckStatus < 10000;
            }
        });
    }

    private void queryStatus() {
        this.mDispatcher.enqueue(new Dispatcher.NodeCall("检查升级状态", "", true, this.mDispatcher) { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.2
            @Override // com.keenon.common.utils.Dispatcher.NodeCall
            protected void execute() {
                PeanutSDK.getInstance().subscribe("UpgradeStatusApi", RosOTAImplV2.this.mStatusCallback);
            }
    ...
    private void checkConfig() {
        this.mDispatcher.enqueue(new Dispatcher.NodeCall("检查OTA配置", "", true, this.mDispatcher) { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.3
            @Override // com.keenon.common.utils.Dispatcher.NodeCall
            protected void execute() {
                if (!RosOTAImplV2.this.checkFile() || !RosOTAImplV2.this.checkVersion() || !RosOTAImplV2.this.checkWS()) {
                    RosOTAImplV2.this.updateCheckStatus(RosOTAImplV2.STATUS_CODE_CHECK_FW_ERROR, true);
                } else {
                    RosOTAImplV2.this.updateCheckStatus(10003, false);
                }
            }
    ...
    private void startTFTPServer() {
        this.mDispatcher.enqueue(new Dispatcher.NodeCall("启动文件服务", "", true, this.mDispatcher) { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.4
            @Override // com.keenon.common.utils.Dispatcher.NodeCall
            protected void execute() {
                RosOTAImplV2.this.updateCheckStatus(10004, true);
            }
    ...
    private void doAction(final String action, UpgradeActionApi.Package data) {
        new UpgradeActionApi().send(new IDataCallback() { // from class: com.keenon.sdk.sensor.ota.impl.RosOTAImplV2.7
            @Override // com.keenon.sdk.external.IDataCallback
            public void success(String result) {
                RosOTAImplV2.this.log("doAction success-" + action, result);
            }

            @Override // com.keenon.sdk.external.IDataCallback
            public void error(ApiError error) {
                RosOTAImplV2.this.log("doAction error-" + action, error.toString());
            }
        }, action, data);
    }
